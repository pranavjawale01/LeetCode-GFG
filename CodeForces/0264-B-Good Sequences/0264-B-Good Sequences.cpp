...
void pranavjawale01() {
    ...
    vll arr(n);
    input1d(arr);
    ll maxEl = *max_element(all(arr));
    ll ans = 0;
    vll dp(maxEl + 1, 0), div(maxEl + 1, 0);
    for (ll i = 0; i < n; i++) {
        dp[arr[i]] = div[arr[i]] = 1;
        for (ll j = 2; j * j <= arr[i]; j++) {
            if (arr[i] % j == 0) {
                dp[arr[i]] = max(dp[arr[i]], div[j] + 1);
                dp[arr[i]] = max(dp[arr[i]], div[arr[i]/j] + 1);
            }
        }
        for (ll j = 2; j * j <= arr[i]; j++) {
            if (arr[i] % j == 0) {
                div[j] = dp[arr[i]];
                div[arr[i]/j] = dp[arr[i]];
            }
        }
